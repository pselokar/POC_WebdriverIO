...
global.postToSlack = function (){
    return new Promise((resolve,reject)=>{
        var parser = new xml2js.Parser();
        logger.info("Initiating consolidation of test results....");
        fs.readFile('./reports/results.xml', function(err, data) {
            parser.parseString(data, function (err, result) {
                var testExecutiontitle = "WebdriverIO Automation Test results:\n Environment URL: "
                logger.info("WebdriverIO Automation Test results:\n URL: ");
                var lenSuite = result.testsuites.testsuite.length;
                logger.info("Total Test Suite Count: "+ lenSuite);
                ...
                for(var j=0; j<lenSuite; j++){
                    ...
                    testSuiteSummary = testSuiteSummary +"   -  "+"_Passed: "+passed.toString()+"_"+"`Failed: "+failed.toString()+
                ...
                logger.info('Posting test summary to slack....');
                ...
                httpRequest(reqOptions).then( function(httpResponse) {
                    logger.info('Response after posting to slack: \n' + httpResponse.toString());
